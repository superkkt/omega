...
func (p *memMIMEPart) RawContent() []byte {
	return p.raw
}

func (p *memMIMEPart) SetRawContent(raw []byte) {
	p.raw = raw
}
...
func parseParts(parent MIMEPart, reader io.Reader, boundary string) error {
	...
	for {
		...
		if boundary != "" {
			...
			tee := io.TeeReader(mrp, &buf)
			// Content is text or data, decode it
			data, err := decodeSection(mrp.Header.Get("Content-Transfer-Encoding"), tee)
			if err != nil {
				return err
			}
			p.SetContent(data)
			p.SetRawContent(buf.Bytes())
		}
	}
